...
Matematiikassa pyritään \termi[todistamaan]{todistaminen} matemaattisia tuloksia.
...
käsiteltävän teorian \termi[aksioomiin]{aksiooma} eli perusoletuksiin sekä loogisiin päättelysääntöihin.
Matematiikassa tärkeimpiä tuloksia sanotaan yleensä \termi[lauseiksi]{lause} eli
\termi[teoreemoiksi]{teoreema} (esim. Pythagoraan lause) ja pienempiä aputuloksia kutsutaan
\termi[apulauseiksi]{apulause} eli \termi[lemmoiksi]{lemma}.
...
\begin{esimerkki}
	...
	\textbf{Lause 1.} Olkoot $m$ ja $n$ parillisia kokonaislukuja. Tällöin luku $m+n$ on parillinen.
	\begin{todistus}
...
\begin{esimerkki}
	...
	\textbf{Lause 2.} Kahden rationaaliluvun summa on rationaalinen.
	\begin{todistus}
...
\begin{esimerkki}
	...
	\textbf{Lause 3.} Olkoot $a$ ja $b$ reaalilukuja. Tulo $ab<0$, jos ja vain jos joko
	$a>0$ ja $b<0$ tai $a<0$ ja $b>0$.
	\begin{todistus}
...
\termi[Käänteisessä todistuksessa]{käänteinen todistus} ajatuksena on, että väitteen $A\to B$
...
\begin{esimerkki}
	...
	\textbf{Lause 4.} Olkoon $m\in \N$ siten, että $m^2$ on parillinen. Tällöin $m$ on parillinen.
...
\termi[Ristiriitatodistuksessa]{ristiriitatodistus} pyritään todistamaan muotoa $A\to B$ oleva
...
aina esittää \termi[supistetussa muodossa]{supistettu muoto} $q=m/n$, missä $m$ ja $n$ ovat kokonaislukuja, joilla ei 
...
\begin{esimerkki}
	...
	\textbf{Lause 5.} Luku $\sqrt{2}$ on irrationaalinen.
...
\begin{enumerate}
	...
	\item Niin sanotun \emph{Fermat'n suuren lauseen} mukaan ei ole
		...
		kokonaislukuja. \begin{enumerate}[a)]
