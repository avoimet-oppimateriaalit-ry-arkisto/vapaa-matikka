...
\begin{esimerkki}
Laske kertolaskut \quad a) $3x\cdot 2x$ \quad b)$-3x^2\cdot (-5x^4)$ \quad 
c) $5x^2 \cdot (-2x)$
\begin{enumerate}[a)]
    \item $3x\cdot 2x = 2\cdot 3\cdot x\cdot x = 6x^2$
...
Toinen yksinkertainen tapaus on kahden binomin tulo. Tällöin kummallakin ensimmäisen binomin termillä kerrotaan toisen binomin termit. Saadut neljä tuloa lasketaan yhteen. 
...
\begin{tikzpicture}
\draw node[right] {\large $(x+2)(x-5) = x^2-5x+2x-10 = x^2 -3x-10.$};

\pgfmathsetmacro{\klAx}{0.48}
\pgfmathsetmacro{\klBx}{1.15}
\pgfmathsetmacro{\klCx}{1.8}
\pgfmathsetmacro{\klDx}{2.5} % oli 2.8
...
\begin{tikzpicture}
\draw node[right] {\large $(x^2-x)(2x-1) = 2x^3-x^2-2x^2+x = 2x^3 -3x^2 +x.$};

\pgfmathsetmacro{\klAx}{0.48}
\pgfmathsetmacro{\klBx}{1.35}
\pgfmathsetmacro{\klCx}{2.0}
\pgfmathsetmacro{\klDx}{2.85}
\pgfmathsetmacro{\klEx}{3.64}
\pgfmathsetmacro{\klLo}{-0.22}
\pgfmathsetmacro{\klHi}{0.26}
