...
&= a^2+ab+ab+b^2 &\emph{kertolaskun vaihdannaisuus, $ba=ab$} \\
&= a^2+2ab+b^2
\end{align*}
...
Edellä kutsuttuja tapauksia kutsutaan yhdessä nimellä binomin neliö, ja muistikaavat voidaan kirjoittaa myös yhtenä yhtälönä muodossa $(a \pm b)^2=a^2 \pm 2ab+b^2$. Kaavoissa monta kertaa esiintyviä plus–miinus-merkkejä luetaan siten, että ylemmät ja alemmat täsmäävät keskenään. Esitetty kaava sisältää siis sekä summan neliön (käytetään ylempiä merkkejä eli vain plussia) että erotuksen neliön (käytetään vain alempia merkkejä eli miinuksia).
...
Esimerkiksi luvun $12$ \termi[tekijät]{tekijä} ovat $1$, $2$, $3$, $4$, $6$ ja $12$. Nämä ovat sellaisia
lukuja, joista saadaan $12$ kertomalla ne jollain kokonaisluvulla. Sanotaan myös, että luvun $12$
\termi[alkutekijät]{alkutekijä} ovat $2$, $2$ ja $3$, koska luku $12$ voidaan
...
Polynomin tekijöitä voi etsiä soveltamalla osittelulakia (sulkujen auki kertomista) käänteisesti:
...
%\begin{tehtava}
%    Olkoon annettu funktio $P: \mathbb{R} \rightarrow \mathbb{R}$, $P(x)=-x^4+2x$. Sievennä lausekkeet.
%    \begin{enumerate}[a)]
%		\item $P(\sqrt{2})$        
%        \item $P(x)^2$
%        \item $P(P(x))$
